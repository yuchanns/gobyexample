...
import (
	...
	"github.com/yuchanns/gobyexample/ent/ent/predicate"
...
type TodoMutation struct {
	...
	_Children        map[int]struct{}
	removed_Children map[int]struct{}
	cleared_Children bool
	parent           *int
	clearedparent    bool
	done             bool
...
// AddChildrenIDs adds the "Children" edge to the Todo entity by ids.
func (m *TodoMutation) AddChildrenIDs(ids ...int) {
	if m._Children == nil {
		m._Children = make(map[int]struct{})
	}
	for i := range ids {
		m._Children[ids[i]] = struct{}{}
	}
}

// ClearChildren clears the "Children" edge to the Todo entity.
func (m *TodoMutation) ClearChildren() {
	m.cleared_Children = true
}

// ChildrenCleared reports if the "Children" edge to the Todo entity was cleared.
func (m *TodoMutation) ChildrenCleared() bool {
	return m.cleared_Children
}

// RemoveChildrenIDs removes the "Children" edge to the Todo entity by IDs.
func (m *TodoMutation) RemoveChildrenIDs(ids ...int) {
	if m.removed_Children == nil {
		m.removed_Children = make(map[int]struct{})
	}
	for i := range ids {
		delete(m._Children, ids[i])
		m.removed_Children[ids[i]] = struct{}{}
	}
}
...
func (m *TodoMutation) RemovedChildrenIDs() (ids []int) {
	for id := range m.removed_Children {
		ids = append(ids, id)
	}
	return
}
...
func (m *TodoMutation) ResetChildren() {
	m._Children = nil
	m.cleared_Children = false
	m.removed_Children = nil
}
...
func (m *TodoMutation) AddedIDs(name string) []ent.Value {
	...
	case todo.EdgeChildren:
		ids := make([]ent.Value, 0, len(m._Children))
		for id := range m._Children {
			ids = append(ids, id)
		}
		return ids
...
func (m *TodoMutation) RemovedIDs(name string) []ent.Value {
	switch name {
	case todo.EdgeChildren:
		ids := make([]ent.Value, 0, len(m.removed_Children))
		for id := range m.removed_Children {
			ids = append(ids, id)
		}
		return ids
